...
While the fixed prior on the missing lag terms worked, I'm not sure it's learning from the data at all. To do that, I need to model the parameters with hyperpriors.

```{r results='show'}
writeLines(readLines("arb_1.6.2.stan"))
```

```{r}
if (params$load_saved) {
  m162.sp <- readRDS("fit_arb_1.6.2_sputum.rds")
} else {
  m162.sp <- stan(
    file="arb_1.6.2.stan", data=d161.spl, iter=2000, chains=4,
    control=list(max_treedepth=17, adapt_delta=0.99))
  saveRDS(m162.sp, file="fit_arb_1.6.2_sputum.rds")
}
```
...
```{r}
s162.sp <- summary(m162.sp)$summary %>% as.data.frame() %>% 
  rownames_to_column("parameter") %>% as.tibble()
s162.sp %>% filter(Rhat >= 1.1 | n_eff < 100) %>% kable()
```

```{r tidy=T}
bayesplot::mcmc_trace(
  rstan::extract(m162.sp, inc_warmup=T, permuted=F),
  pars = c(
    "mu",
    "sigma_spec",
    "sigma_subj",
