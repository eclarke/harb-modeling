...
d1.5.0.sp <- d2 %>% filter(specimen_type == "Sputum") %>%
  ...
  mutate(on_abx = effective_window(abx_yn, lag=0, lead=3)) %>%
...
d %>% group_by(subject_id) %>%
  distinct(subject_id, study_day, abx_yn) %>%
  arrange(subject_id, study_day) %>%
  tidyr::complete(study_day=full_seq(study_day, 1)) %>%
  mutate(on_abx = effective_window(abx_yn, lag=0, lead=2)) %>%
  ggplot(aes(x=as.factor(study_day), y=subject_id, fill=abx_yn)) +
