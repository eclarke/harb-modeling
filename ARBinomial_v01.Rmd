...
```{r}
d1.4.4 <- d %>% filter(specimen_type=="Oral Swab") %>%
  group_by(subject_id) %>%
  filter(n() > 1) %>%
  ungroup() %>%
  mutate(abx_yn = !is.na(abx_b)) %>%
  ...
  mutate(study_day = study_day) %>%
  mutate(emp_prob = read_count/total_reads)
d1.4.4a <- d1.4.4 %>% arrange(subject_id, study_day) %>% compose_data()
d1.4.4a$n_obs_subject <- (d1.4.4 %>% group_by(subject_id) %>% summarize(n = n()))$n

m1.4.4a <- stan(
  file="arb_1.4.4.stan", data=d1.4.4a, iter=2000, chains=4,
  control=list(max_treedepth=17, adapt_delta=0.95))
saveRDS(m1.4.4a, file = "fit_arb_1.4.4a.rds")
m1.4.4b <- readRDS("fit_arb_1.4.4a.rds")
```
...
invisible(with(data.frame(), {
  ...
  .m <- m1.4.4a
  .d <- d1.4.4
  post <- spread_samples(.m, mu, b_abx[i], a_subj[i])
  est <- mean_qi(post)
  plot(logistic(est$mu + est$a_subj))
  plot(logistic(est$mu + est$a_subj + est$b_abx))
  plot(logistic(est$b_abx))
  logistic(est$mu + est$a_subj + est$b_abx)
  

  pred <- matrix(data=NA, nrow = 4000, ncol=nrow(.d))
emp_pred1.2 <- pred1.2
pred1.2[,1] <- d1.2$read_count[1]
emp_pred1.2[,1] <- d1.2$emp_prob[1]
for (i in 2:nrow(d1.2)) {
