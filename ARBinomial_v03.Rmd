...
dat205.generator <- function(d2, .specimen_type, min_times_abx=1) {
  ...
  abx_to_keep <- filter(abx_tally, n_subjects > 3, abx_b != "none")$abx_b
  subjects_to_keep <- (d3 %>% filter(abx_b %in% abx_to_keep) %>% distinct(subjects))$subjects

  d4 <- d3 %>% ungroup() %>%
    filter(subjects %in% subjects_to_keep)
  d_abx <- d4 %>%
    reshape2::dcast(specimens + subjects + reads + total + prev ~ abx_b, value.var="abx_yn", fill=0)
...
d205.3 <- dat205.generator(d2, "Sputum", 40)
...
m205.3 <- stan(
  file="arb_2.0.5.stan", data=d205.3, cores=4, iter=500, chains=4,
